...
export const blogPost = catchAsyncError(async (req, res, next) => {
    ...
    if (!allowedFormats.includes(mainImage.mimetype) || (paraOneImage && !allowedFormats.includes(paraOneImage.mimetype)) || (paraTwoImage && !allowedFormats.includes(paraTwoImage.mimetype)) || (paraThreeImage && !allowedFormats.includes(paraThreeImage.mimetype))) {
        return next(new ErrorHandler("Invalid file type only jpeg png and webp formats are allowed"))
    }
    ...
    //promises array , array khud execute ni hota isliye humne
    const uploadPromises = [cloudinary.uploader.upload(mainImage.tempFilePath), 
     ...
     //isliye humne ye bnaya promise.all se execute kraya
    const [mainImageRes, paraOneImageRes, paraTwoImageRes, paraThreeImageRes,] = await Promise.all(uploadPromises);

    if (!mainImageRes || mainImageRes.error || (paraOneImage && (!paraOneImageRes || paraOneImageRes.error)) || (paraTwoImage && (!paraTwoImage || paraTwoImageRes.error)) || (paraThreeImage && (!paraThreeImageRes || paraThreeImageRes.error))) {
